...
Bandit_Lin = {'Arm':Arm, 'Reward':Reward, 'Cum_regret': cum_regret, 'Prob_opt':prob_opt, 'UCB':[]}
...
for t in range(T):
    ...
    Bandit_Lin['Reward'].append(rt)
    Bandit_Lin['Cum_regret'].append(sum_regret)
    Bandit_Lin['Prob_opt'].append(prob_opt)
    Bandit_Lin['UCB'].append(UCB_t[at])
    b += rt * x_ta
    A += x_ta * x_ta.T
    print(round(t/T,3))

Bandit_Lin = pd.DataFrame.from_dict(Bandit_Lin)
#
# # Graphical illustration
# f, ax = plt.subplots(2, sharex='all')
# ax[0].set_title('Cumulative regret')
# ax[0].plot(Bandit_Lin['Cum_regret'])
#
# ax[1].set_title('Probability of choosing opt arm')
# ax[1].plot(Bandit_Lin['Prob_opt'])